...
export default function OrganizationSignUp() {
    ...
    const [account_no,setAccount] = useState(undefined)
    ...
    async function handleSubmit() {
        console.log(data)
        const res = await addOrganization({...data, account_no:account_no});
        if (res) {
            SwlSuccess();
            navigate.push('/login')
        }
    ...
    return (
        <>{!vState ?
            ...
            :
            <>
                <Verificaton setAccount={setAccount} handleSubmit={handleSubmit} vCode={vCode} />
            </>
        }
        </>
    )
