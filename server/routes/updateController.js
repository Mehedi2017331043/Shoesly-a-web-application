...
module.exports.addToLocal = async (req, res) => {
    try {
        ...
        const result = await Organization.findById(req.body.data.prodId);
        ...
        console.log(result2);
        if (result2[0] && result) {
            result2[0].quantity = parseInt(result2[u0].quantity) - parseInt(req.body.data.quantity);
            result2[0].save(err => {
                if (!err) {
                    res.status(200).json(true)
                }
            ...
            const inventory = await new LocalInventory(data);
            await inventory.save(err => {
                if (!err) {
                    res.status(200).json(true)
                }
                else {
...
module.exports.accpetUserOrder = async (req, res) => {
    try {
        ...
        if (result.length !== 0 && result2) {
            ...
            console.log(res2);
            if (res2) {
                const res3 = await User.findOne({account_no:"123456"});
                res3.balance= 0.05*(parseInt(res3.balance)+ parseInt(res2.quantity* res2.price));
                
                await res3.save();
                if(res3){
                    const res4 = await Product.findOne({_id:res2.productId});
                    if(res4){
                        const res5 = await Organization.findById(res4.oid);
                        res3.balance= 0.95*(parseInt(res3.balance)+ parseInt(res2.quantity* res2.price));
                    }
                }
            }
...
module.exports.createCard = async (req,res)=>{
    try {
        ...
        const CVV = data.cvv;
        const res1 = await User.findOne({name:data.name_on_card})
        console.log(res1)
        if(res1){
            const card = await Card.create({
                ref_id: data.name_on_card,
                secret: secret,
